...
const ChatSeller = () => {
    ...
    return (
        <div className='px-2 lg:px-7 py-5'>
            <div className='w-full bg-[#fff] px-4 py-4 rounded-md h-[calc(100vh-140px)]'>
                <div className='flex w-full h-full relative'>
                    <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#fff] md:bg-transparent overflow-y-auto'>
                            ...
                            {
                                sellers.map((s, i) => <Link key={i} to={`/admin/dashboard/chat-sellers/${s._id}`} className={`h-[60px] flex justify-start gap-2 items-center text-black px-2 rounded-sm py-2 rounded-sm cursor-pointer ${sellerId === s._id ? 'bg-[#d5d5da]' : ''}`}>
                                    <div className='relative'>
                                        <img className='w-[38px] h-[38px] border-black border-2 max-w-[38px] p-[2px] rounded-full' src={s.image} alt="" />
                                        {
                    ...
                    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
                        ...
                        <div className='py-4'>
                            <div className='bg-[#d5d5da] h-[calc(100vh-290px)] rounded-md p-3 overflow-y-auto'>
                                {
                        ...
                        <form onSubmit={send} className='flex gap-3'>
                            <input value={text} onChange={(e) => setText(e.target.value)} readOnly={sellerId ? false : true} className='w-full flex justify-between px-2 border border-slate-700 items-center py-[5px] focus:border-blue-500 rounded-md outline-none bg-[#d5d5da] bg-transparent text-[#000000]' type="text" placeholder='input your message' />
